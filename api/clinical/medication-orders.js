...
function groupBy(rows = [], key = 'order_id') {
...
async function hydrateOrders(supabase, orders) {
  ...
  const groupedItems = groupBy(itemsResult.data || [])
  const groupedChecks = groupBy(checksResult.data || [])
...
async function createMedicationOrder(supabase, body, res) {
  ...
  const cleanItems = items.map((item) => ({
    id: makeId('meditem'),
    order_id: order.id,
    medicine_name: medicineName(item),
...
async function createSummary(supabase, body, res) {
  ...
  const row = {
    ...
    patient_friendly_summary: clean(body.patientFriendlySummary, 8000) || buildPatientSummary(body),
    status,
